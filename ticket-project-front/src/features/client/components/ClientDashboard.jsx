...
export default function ClientDashboard() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSuccess(
        `El ticket ha sido creado correctamente con vencimiento el ${formatDate(
          created.limitDateSla
        )}.`
      );
      setForm(initialForm);
      await loadTickets();
        ...
        <section className="my-tickets">
          ...
          {loading ? (
            ...
            <div className="client-ticket-list">
              {tickets.map((ticket) => {
                const state = TICKET_STATUS[ticket.state];
                const priority = TICKET_PRIORITIES[ticket.priority];

                return (
                  <article className="client-ticket" key={ticket.id}>
                    ...
                    <div className="ticket-badges">
                      <Badge
                        text={state?.label || ticket.state}
                        bgVar={state?.bgVar}
                        textVar={state?.textVar}
                        icon={state?.icon}
                      />
                      <Badge
